@extends('layouts.app')
@section('title', "Admin Dashboard — EAPCE'27")
...
<div style="background:linear-gradient(160deg,#060e1c,#0c1f35 70%);padding:60px 0 40px">
    <div class="container" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px">
        ...
        <form method="POST" action="{{ route('admin.logout') }}">
            @csrf
            <button type="submit" class="btn-outline" style="padding:8px 18px;font-size:0.8rem;border-color:rgba(255,255,255,0.3)">
                Sign Out
            </button>
        </form>
    </div>
    ...
    <div class="container">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:16px;margin-bottom:36px">
            ...
            <div class="admin-stat" style="grid-column:span 2">
                <div class="admin-stat-num" style="color:var(--green)">USD {{ number_format($stats['revenue']) }}</div>
        ...
        <div class="card" style="padding:24px;margin-bottom:32px">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:8px">
                ...
                <div style="background:var(--off-white);border:1px solid var(--light-gray);border-radius:6px;padding:6px 14px;font-size:0.82rem">
                    <strong style="color:var(--navy)">{{ $country }}</strong>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <span style="color:var(--gray);font-size:0.78rem">{{ $r->email }}</span>
                        </td>
                        <td>{{ $r->organisation }}</td>
                        <td>{{ $r->country }}</td>
                        <td>
                            <span class="badge" style="background:#eff6ff;color:#1e40af;text-transform:capitalize">
                                {{ $r->reg_type }}
                            </span>
                        </td>
                    ...
                    @empty
                    <tr>
                        <td colspan="9" style="text-align:center;color:var(--gray);padding:32px">
